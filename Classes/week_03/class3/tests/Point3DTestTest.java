...
class Point3DTestTest {
    ...
    public void equalsTest() {
        ...
        for(int i=0;i<size;i++) {
            ...
            Assertions.assertNotEquals(p1.toString(),Point3D.ORIGIN.toString());
        }
    }
    ...
    public void pointLineTest() {
        ...
        if(v!=expected) {
            Assertions.fail("ERR: the pointLineTest should return error on null input, got: "+v);
        }
        b = new Point3D(p);
        ...
        v = p.pointLineTest(a,b);
        if(v!=expected) {
            Assertions.fail("ERR: the pointLineTest should return error on a==b input, got: "+a+", "+b);
        }
    }
    @Test
